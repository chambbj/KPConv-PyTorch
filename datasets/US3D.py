...
class US3DDataset(PointCloudDataset):
    """Class to handle US3D dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'US3D')
...
class US3DSampler(Sampler):
    """Sampler for US3D"""

    def __init__(self, dataset: US3DDataset):
...
class US3DCustomBatch:
    """Custom batch definition with memory pinning for US3D"""
...
def US3DCollate(batch_data):
    return US3DCustomBatch(batch_data)
